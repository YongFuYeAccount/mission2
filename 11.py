...
amp0_train = np.load('./dataset/dataset_3/train_csi_a.npy')#(176712, 62)
amp0_test = np.load('./dataset/dataset_3/test_csi_a.npy')#(68192, 62)

kinect_train = np.load('./dataset/dataset_3/train_kinect.npy',allow_pickle=True)#(88356, 50)
kinect_test = np.load('./dataset/dataset_3/test_kinect.npy',allow_pickle=True)#(34096, 50)

# pha0_train = np.load('./dataset/dataset_2/train_csi_p.npy')#(201178, 62)
# pha0_test = np.load('./dataset/dataset_2/test_csi_p.npy')#(77626, 62)


# print(pha0_train.shape)
# print(pha0_test.shape)

# print('kinect_train.shape',kinect_train.shape)
# print('kinect_test.shape',kinect_test.shape)
# print('amp0_train.shape',amp0_train.shape)
# print('amp0_test.shape',amp0_test.shape)
# print('',type(amp0_train))
#
#
...
train_data = amp0_train
test_data = amp0_test

train_label = kinect_train
test_label = kinect_test
trainX = train_data.reshape((-1,1,4,31))
testX = test_data.reshape((-1,1,4,31))
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(         # input shape (1, 8, 8)维度 图片的宽和高
            nn.Conv2d(
                ...
                padding=4,                  # if want same width and length of this image after Conv2d,
        ...
        self.conv2 = nn.Sequential(         # input shape (16, 4, 4)
            nn.Conv2d(16, 32, (3,5), 1, 4),     # output shape (32, 4, 4)
            # nn.Conv2d(16, 32, 5, 1, 2),
            nn.ReLU(),                      # activation
            nn.MaxPool2d(2),                # output shape (32, 2, 2)
        )
        self.out = nn.Linear(1600, 50)   # fully connected layer, output 5 classes
